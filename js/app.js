...
function renderSidebar(role) {
    ...
    // 1.5. Courseworks -> CW (NEWLY ADDED)
    const courseworkBtn = document.createElement('button');
    courseworkBtn.className = "w-full text-left px-6 py-2 text-slate-400 hover:bg-slate-900 hover:text-orange-400 transition-colors flex items-center gap-0 border-l-2 border-transparent hover:border-orange-500";
    courseworkBtn.innerHTML = createMenuLabel('fa-book-open', 'Courseworks', 'CW');
    courseworkBtn.onclick = () => {
        renderCourseworksPage(); 
        closeMobileSidebar();
    };
    qaSubmenu.appendChild(courseworkBtn);
...
function renderCourseworksPage() {
    elements.pageTitle().innerText = "Courseworks";
    const content = elements.contentArea();
    content.innerHTML = '';
    if (typeof renderQuizzesAndActivities === 'function') {
        // Filter: Looks for 'coursework' in the name/type
        renderQuizzesAndActivities(content, currentUser, renderAccountingCycleActivity, 'Coursework');
    } else {
        content.innerHTML = `<div class="p-8 text-center text-gray-500">Module not loaded properly.</div>`;
    }
}
