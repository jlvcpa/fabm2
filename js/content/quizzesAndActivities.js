...
import { renderTeacherReviewDashboard } from "./activityResultPreview.js";
    ...
    contentArea.innerHTML = `
        <div class="flex h-full relative overflow-hidden bg-gray-50">
            <div id="qa-sidebar" class="w-full md:w-80 bg-white border-r border-gray-200 flex flex-col h-full z-10 transition-transform absolute md:relative transform -translate-x-full md:translate-x-0">
                <div class="p-4 border-b border-gray-200 bg-blue-900 text-white">
                    ...
                    <div id="teacher-filter-area" class="${user.role === 'teacher' ? '' : 'hidden'}">
                        <select id="section-filter-dropdown" class="w-full p-2 rounded bg-blue-800 text-white text-xs border border-blue-700 focus:outline-none focus:ring-1 focus:ring-blue-400">
            ...
            <div id="qa-runner-container" class="flex-1 overflow-hidden relative bg-gray-100 flex">
...
async function loadStudentActivities(user, customRunner, filterType) {
    ...
    try {
        ...
        snapshot.forEach(docSnap => {
            ...
            card.className = `p-3 rounded border cursor-pointer hover:shadow-md transition bg-white border-blue-200 mb-2`;
            
            card.innerHTML = `
                <div class="flex justify-between items-start">
                    <h3 class="font-bold text-gray-800 text-xs">${data.activityname}</h3>
                    <span class="text-[10px] bg-gray-100 px-1 rounded">${data.section}</span>
                </div>
                <p class="text-[10px] text-gray-400 mt-1">${isPerformanceTask ? 'Performance Task' : 'Standard Quiz'}</p>
            `;
            ...
            card.onclick = () => {
                ...
                if (user.role === 'teacher') {
                    renderStudentSelectionList(data, user);
                } else {
        ...
        console.error(e);
        listContainer.innerHTML = '<p class="text-center text-red-400 mt-4 text-xs">Error loading data.</p>';
    }
}
 ...
 */
async function renderStudentSelectionList(activityDoc, teacherUser) {
    const container = document.getElementById('qa-runner-container');
    container.innerHTML = `
        <div class="w-72 bg-white border-r border-gray-200 flex flex-col h-full shrink-0">
            <div class="p-4 bg-gray-50 border-b">
                <h3 class="font-bold text-sm text-blue-900 truncate">${activityDoc.activityname}</h3>
                <p class="text-[10px] text-gray-500">Student Submissions</p>
            </div>
            <div id="student-list-items" class="flex-1 overflow-y-auto">
                <div class="p-8 text-center"><i class="fas fa-spinner fa-spin text-blue-900"></i></div>
            </div>
        </div>
        <div id="result-preview-content" class="flex-1 bg-gray-100 overflow-y-auto p-6">
            <div class="h-full flex flex-col items-center justify-center text-gray-400">
                <i class="fas fa-user-graduate text-5xl mb-4 opacity-20"></i>
    ...
    try {
        ...
        if (snap.empty) {
            listItems.innerHTML = '<p class="p-4 text-center text-gray-400 text-xs">No one has submitted yet.</p>';
            return;
        }
        ...
        snap.forEach(docSnap => {
            ...
            btn.className = "w-full text-left p-3 border-b hover:bg-blue-50 transition-colors flex items-center gap-3 group";
            btn.innerHTML = `
                <div class="w-6 h-6 rounded-full bg-gray-200 flex items-center justify-center text-[10px] font-bold text-gray-600 group-hover:bg-blue-200">${res.CN || '#'}</div>
                <div class="truncate flex-1">
                    <div class="text-xs font-bold text-gray-800">${res.studentName}</div>
                    <div class="text-[10px] text-gray-400">${new Date(res.timestamp).toLocaleDateString()}</div>
                </div>
                <i class="fas fa-chevron-right text-[10px] text-gray-300"></i>
            `;
            
            btn.onclick = () => {
                const previewArea = document.getElementById('result-preview-content');
                previewArea.innerHTML = '<div class="p-10 text-center"><i class="fas fa-spinner fa-spin text-3xl text-blue-900"></i></div>';
                
                // Construct a user-like object for the preview component
                const studentUserObj = {
                    FirstName: res.studentName.split(', ')[1] || '',
                    LastName: res.studentName.split(', ')[0] || '',
                    Idnumber: res.studentId,
                    CN: res.CN,
                    role: 'student'
                };

                // Trigger the capability of activityResultPreview.js
                renderTeacherReviewDashboard(previewArea, teacherUser, activityDoc, res, collectionName, docSnap.id);
            };
            listItems.appendChild(btn);
        });
    } catch (e) {
        listItems.innerHTML = `<p class="p-4 text-red-500 text-xs">Error: ${e.message}</p>`;
    }
}

// ... [Remainder of your existing functions: renderQuizRunner, generateQuizContent, etc. remain intact] ...
