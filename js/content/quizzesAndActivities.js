...
async function generateQuizContent(activityData, savedState = null) {
    ...
    for (const [index, section] of activityData.testQuestions.entries()) {
        ...
        questions.forEach((q, qIdx) => {
            ...
            if (section.type === "Journalizing and Preparing SCE (Corp)") {
            ...
            // ORIGINAL MULTIPLE CHOICE & PROBLEM SOLVING
            else {
        ...
        if (section.type !== "Journalizing" && section.type !== "Journalizing and Preparing SCE (Corp)") {
            sectionsHtml += `
...
function initializeQuizManager(activityData, questionData, user, savedState) {
    ...
    sections.forEach(section => {
        ...
        if (type !== 'Journalizing' && type !== 'Journalizing and Preparing SCE (Corp)') {
    ...
    function checkCompletion() {
        ...
        for (const q of questionData) {
            ...
            } else if (q.type === 'Journalizing and Preparing SCE (Corp)') {
                // Determine completion: If ANY JE input is filled OR ANY SCE input is filled, it's partially answered.
                // We'll require at least one input in both sections to consider it "complete enough" to allow submit.
                const jeInputs = form.querySelectorAll(`input[name^="${q.uiId}_JE_"]`);
                const sceInputs = form.querySelectorAll(`input[name^="${q.uiId}_SCE_"], select[name^="${q.uiId}_SCE_"]`);
...
async function saveProgress(activityData, questionData, user) {
    ...
    questionData.forEach(q => {
        ...
        if(q.type === 'Multiple Choice') { 
            ...
            // Save JE
            const jeInputs = document.querySelectorAll(`input[name^="${q.uiId}_JE_"]`);
            jeInputs.forEach(input => {
                if(input.value) hasIntegratedData = true;
                const parts = input.name.split('_'); 
                // format: s0_q0_JE_t0_r0_date
                const tIdx = parts[3];
                const rIdx = parts[4];
                const field = parts[5];
                const key = `${tIdx}_${rIdx}`;
            ...
            // Save SCE
            const sceInputs = document.querySelectorAll(`input[name^="${q.uiId}_SCE_"], select[name^="${q.uiId}_SCE_"]`);
            sceInputs.forEach(input => {
                if(input.value) hasIntegratedData = true;
                const parts = input.name.split('_');
                // format: s0_q0_SCE_r0_date
                const rIdx = parts[3];
                const field = parts[4];
        ...
        // Check for already saved (disabled) values (Legacy)
        ...
        // For new integrated type, saved values are natively picked up by the selector without disabled filtering
        // so `value` already contains everything.
...
async function submitQuiz(activityData, questionData, user, isFinal = false, forceSubmit = false) {
    ...
    questionData.forEach(q => {
        ...
        // Catch legacy disabled elements for simple types
    ...
    questionData.forEach(q => {
        if (q.type === 'Journalizing and Preparing SCE (Corp)') {
            ...
            // JE Grading
            if (q.transactions) {
                q.transactions.forEach((tx, tIdx) => {
                    if (tx.solution) {
                        tx.solution.forEach((row, rIdx) => {
                            if(row.isExplanation) return;
                            const cellKey = `t${tIdx}_r${rIdx}`;
                            const studentRow = (val && val.JE && val.JE[cellKey]) ? val.JE[cellKey] : {};
            ...
            expectedSce.forEach((expRow, rIdx) => {
                const studentRow = (val && val.SCE && val.SCE[`r${rIdx}`]) ? val.SCE[`r${rIdx}`] : {};
                
                // Date
                if (expRow.date !== undefined && expRow.date !== null && String(expRow.date).trim() !== '') {
                    sceMax++;
                    if(studentRow.date && String(studentRow.date).trim().toLowerCase() === String(expRow.date).trim().toLowerCase()) sceScore++;
                }
                
                // Description
                if (expRow.desc && String(expRow.desc).trim() !== '') {
                    sceMax++;
                    if(studentRow.desc && String(studentRow.desc).trim().toLowerCase() === String(expRow.desc).trim().toLowerCase()) sceScore++;
                }
            ...
            // Update main section scores
            sectionScores[index].score += (jeScore + sceScore);
            sectionScores[index].maxScore += (jeMax + sceMax);
            
            // Re-calculate percentage inline
            ...
            // Store granular details in answers for later review
            answers[q.uiId] = {
                ...val,
                gradingDetails: { jeScore, jeMax, sceScore, sceMax }
            };
        }
    });
