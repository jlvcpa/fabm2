...
// IMPORT DRY HANDLER
import { buildExpectedSce, IntegratedSceHandler } from './activityHandlers/integratedSceHandler.js';
        ...
        activityConfig.testQuestions.forEach((section, idx) => {
            ...
            sectionQs.forEach(q => {
                ...
                if (section.type === "Multiple Choice") {
                    ...
                    // Implement DRY Method directly mapped to the robust Handler
                    const result = IntegratedSceHandler.grade(q, studentAns, liveQ);
                    secScore += result.score;
                    secMax += result.maxScore;
                    q.computedScores = result.details;
                }
            });
                                    ...
                                    return html`
                                        <div key=${q.uiId} className="border rounded p-4 bg-white mb-6 shadow-sm">
                                            ...
                                            <div className="flex justify-between items-center mb-3">
                                                ...
                                                <span className="text-sm font-bold text-indigo-700 bg-indigo-50 px-3 py-1 rounded border border-indigo-200">Score: ${jeScore} / ${jeMax} pts</span>
                                            </div>
                                            
                                            ${(liveQ.transactions || []).map((trans, tIdx) => {
                                                const solRows = trans.solution || [];
                                                const rowCount = trans.rows || (solRows ? solRows.length : 3);
                                                ...
                                                rawStudentRows.forEach((sr, r) => {
                                                    ...
                                                    if (r < solRows.length) {
                                                        ...
                                                        if (sol.isExplanation) {
                                                            expAcct = true;
                                                            if (sAcct.trim() === String(sol.account).trim()) {
                                                                acctCorrect = true;
                                                            }
                                                        } else {
                                                            if (sol.date !== undefined && sol.date !== null && String(sol.date).trim() !== '') {
                                                                expDate = true;
                                                                let expectedDate = String(sol.date).trim();
                                                                if (tIdx > 0) {
                                                                    const parts = expectedDate.split(' ');
                                                                    if (parts.length > 1) expectedDate = parts[parts.length - 1];
                                                                }
                                                                if (sDate && sDate.toLowerCase() === expectedDate.toLowerCase()) dateCorrect = true;
                                                            }
                                            ...
                                            <div className="flex justify-between items-center mt-8 mb-3 border-t pt-6">
                                                ...
                                                <span className="text-sm font-bold text-purple-700 bg-purple-50 px-3 py-1 rounded border border-purple-200">Score: ${sceScore} / ${sceMax} pts</span>
                                            </div>
