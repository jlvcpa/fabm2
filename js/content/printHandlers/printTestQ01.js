...
export const handlePrintTQ = (activityConfig) => {
    ...
    // B. Completely REMOVE the blue/slate screen headers that contain the scores and "Print Part" buttons
    clone.querySelectorAll('.bg-blue-900.text-white, .bg-slate-800.text-white, .print\\:hidden, .hide-in-print').forEach(el => el.remove());

    // C. Remove all Explanations, Warnings, Answer Keys, and Sticky Top Headers
    clone.querySelectorAll('[id^="ans-"], [id^="msg-"], .sticky, .btn-reveal-set').forEach(el => el.remove());
    
    // D. Remove all validation SVG icons (Checks and X marks)
    ...
    // E. Uncheck all radio buttons and hide them so we only see the text options
    ...
    // G. Strip borders, background colors, and padding from Question Cards & Options
