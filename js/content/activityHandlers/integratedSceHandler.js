...
export function buildExpectedSce(q) {
    ...
    transactions.forEach(tx => {
        ...
        // If transaction has no numerical impact on SCE (like Memo Entry Auth), do not add a row.
    ...
    for (let rIdx = 0; rIdx < maxRowsToCheck; rIdx++) {
        ...
        if (expRow) {
            ...
            if (expRow.date !== undefined && expRow.date !== null && String(expRow.date).trim() !== '') {
                ...
                if (rIdx === 1) { // First transaction row needs full date
                    if (studentDate === expectedFullDate) { sceScore++; sEval.dateCorrect = true; }
                } else { // Subsequent rows can be full or just the day
    ...
    renderQuestion: (q, qIdx, uiId, savedValue, disabledAttr, dimClass, hiddenClass) => {
        ...
        for (let r=0; r < expectedSceRowsCount; r++) {
            ...
            if (isBegRow || isEndRow) {
                dateInputHtml = `<input type="text" name="${uiId}_SCE_r${r}_date" class="w-full p-2 text-center outline-none bg-transparent font-mono text-sm text-gray-400 cursor-not-allowed" value="" readonly tabindex="-1">`;
            } else {
                const datePlaceholder = r === 1 ? "Mmm d" : "d";
                dateInputHtml = `<input type="text" name="${uiId}_SCE_r${r}_date" class="w-full p-2 text-center outline-none bg-transparent font-mono text-sm ${sceDim}" placeholder="${datePlaceholder}" value="${sceData.date !== undefined ? sceData.date : ''}" ${sceDisabledAttr}>`;
            }
