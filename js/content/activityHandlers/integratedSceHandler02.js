...
export function buildExpectedSce(q) {
    ...
    transactions.forEach(tx => {
        ...
        // Strict Exclusion: If no numerical impact occurs (e.g. Memo Auth), do NOT add it to the answer key
        if (hasImpact(impacts.Common)) {
            impacts.Common.desc = determineDesc(impacts.Common, false);
            soceRows.push(impacts.Common);
            commonAdded = true;
        }

        if (hasImpact(impacts.Preferred)) {
    ...
    for (let rIdx = 0; rIdx < maxRowsToCheck; rIdx++) {
        ...
        if (expRow) {
            ...
            if (expRow.date !== undefined && expRow.date !== null && String(expRow.date).trim() !== '') {
                ...
                if(sr.date && String(sr.date).trim().toLowerCase() === String(expRow.date).trim().toLowerCase()) { sceScore++; sEval.dateCorrect = true; }
            }
...
export const IntegratedSceHandler = {
    ...
    renderQuestion: (q, qIdx, uiId, savedValue, disabledAttr, dimClass, hiddenClass) => {
        ...
        transactions.forEach((trans, tIdx) => {
            ...
            for(let r=0; r < rowCount; r++) {
                ...
                let acctClass = "";
                if (cellData.cr && String(cellData.cr).trim() !== '') indentStyle = "padding-left: 1.25rem;";

                let dateCellHtml = ''; let acctCellHtml = ''; let drCellHtml = ''; let crCellHtml = '';
    ...
    grade: (q, studentAns, liveQ) => {
        ...
        return { score: res.scores.score, maxScore: res.scores.maxScore, details: res.scores };
    }
};
